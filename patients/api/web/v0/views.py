...
@extend_schema(
    ...
    description="Retorna lista paginada de pacientes do grupo do usuário logado para aplicação web",
    tags=["Web - Patients"],
    methods=['GET'],
...
@extend_schema(
    operation_id="upload_patient_photo",
    summary="Upload de foto do paciente",
    description="Faz upload de uma nova foto do paciente. A imagem será redimensionada automaticamente para 800x800px mantendo proporção.",
    tags=["Web - Patients"],
    methods=['POST'],
    request={
        'multipart/form-data': {
            ...
            'required': ['patient_photo']
        }
    },
    responses={
        200: inline_serializer(
            name="UploadPatientPhotoResponse",
            fields={
...
@extend_schema(
    ...
    description="Remove a foto atual do paciente.",
    tags=["Web - Patients"],
    methods=['DELETE'],
    responses={
        200: inline_serializer(
            name="DeletePatientPhotoResponse",
            fields={
                "detail": serializers.CharField(),
                "patient": PatientWithPhotoSerializer(),
            },
...
def manage_patient_photo(request, patient_id):
    ...
    # Verificar permissão para modificar este paciente
    if not user.groups.filter(name='system_admin').exists():
        from groups.models import UserClinic, GroupAdmin
        ...
        if not user_in_patient_group:
            return Response(
                {"detail": "Você não tem permissão para modificar a foto deste paciente."},
                status=status.HTTP_403_FORBIDDEN
            )
    ...
    if request.method == 'POST':
        ...
        if serializer.is_valid():
            try:
                # Salva a foto e atualiza o paciente
                patient = serializer.save(patient=patient)
                
                # Retorna dados atualizados do paciente
        ...
        try:
            ...
            # Remove foto do S3 e limpa campo
...
@extend_schema(
    operation_id="get_patient_doctors",
...
def get_patient_doctors(request, patient_id):
    ...
    from users.api.web.v0.patient_relations_serializers import PatientDoctorsListSerializer
